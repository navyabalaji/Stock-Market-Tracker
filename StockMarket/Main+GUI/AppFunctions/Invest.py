...
class InvestStocks:
    def __init__(self):
        ...
        self.symbol_entry = ctk.CTkEntry(self.form_frame, 
                                           width=300, 
                                           placeholder_text="Enter symbol of stock you wish to invest in")
        ...
        self.enter_btn = ctk.CTkButton(self.form_frame, 
                                        ...
                                        corner_radius=10,
                                        fg_color="green",
                                        command=self.invest_guide)
        self.enter_btn.place(relx=0.5, rely=0.55, anchor=ctk.CENTER)
        ...
        self.back_btn = ctk.CTkButton(self.form_frame, 
                                        ...
                                        fg_color="red",
                                        command=self.back)
        self.back_btn.place(relx=0.5, rely=0.65, anchor=ctk.CENTER)

        self.details = None  # To store user-entered details

    def invest_guide(self):
        """
        Gives suggestions concerning investments
        """
        self.details = [((self.symbol_entry.get()).strip()).upper(), self.cmpname_entry.get()]
        with open("Invest.txt","w") as file:
            file.write(((self.symbol_entry.get()).strip()).upper()+'.NS'+'\n'+self.cmpname_entry.get())
        self.root.destroy()
        subprocess.Popen([sys.executable,"Main+GUI/AppFunctions/Invest_backend.py"])
